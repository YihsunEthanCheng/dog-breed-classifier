...
train_tensor_x = bottleneck_features['train']
valid_tensor_x = bottleneck_features['valid']
test_tensor_x = bottleneck_features['test']

#%%
final_model = Sequential()
final_model.add(GlobalAveragePooling2D(input_shape=train_tensor_x.shape[1:]))
...
filepath = 'saved_models/weights.best.'+ netID + '.hdf5'
checkpointer = ModelCheckpoint(filepath=filepath, verbose=1, save_best_only=True)

final_model.fit(train_tensor_x, train_targets, 
          validation_data=(valid_tensor_x, valid_targets),
...
final_predictions = [np.argmax(final_model.predict(np.expand_dims(feature, axis=0))) for feature in test_tensor_x]
...
def predict_dog_breed(img_path):
    ...
    f_tensor = extract_Xception(img_tensor)
#    fvec = np.array([ np.mean(f_tensor[:,:,:,i]) for i in range(f_tensor.shape[-1])])
    nameid = np.argmax(final_model.predict(f_tensor[None,:]))
...
my_file = ['C:\_cloud\dl_nano\dog-project\images\kim.jpg', 
           ...
           'C:\_cloud\dl_nano\dog-project\images\me.jpg']


#%%
import cv2
img = cv2.imread(my_file[-1])
plt.imshow(img)
